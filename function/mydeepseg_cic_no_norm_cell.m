function imageout=mydeepseg_cic_no_norm_cell(imagedata,currentfoldername,resizefactor,shiftmove,datapadsmallbw,multiplev)
    ...
    for yy1=0:(yyrow-shiftmove-shiftmove):yyyy2
        try
            ...
            if (sum(sum(imagecropbw==1))/sum(sum(imagecropbw>-99)))>0.1
                ...
                newmultiimage= uint8(zeros(xxrow,yyrow));
                newmultiimage(C=='pos_nuc')=1;
                newmultiimage(C=='pos_mem')=2;
                newmultiimage(C=='neg_nuc')=3;
                newmultiimage(C=='neg_mem')=4;
                newmultiimage(C=='other')=5;
