function imageout=mydeepseg_cic_Nonorm_derm(imagedata,currentfoldername,resizefactor,shiftmove,datapadsmallbw,multiplev)
...
for xx1=0:(xxrow-shiftmove-shiftmove):xxxx2
    for yy1=0:(yyrow-shiftmove-shiftmove):yyyy2
        try
            ...
            if (sum(sum(imagecropbw==1))/sum(sum(imagecropbw>-99)))>0.05
                ...
                newmultiimage= uint8(zeros(xxrow,yyrow));
                newmultiimage(C=='Epi')=1;
                newmultiimage(C=='dermpap')=2;
                newmultiimage(C=='artefact')=3;
                newmultiimage(C=='other')=4;
